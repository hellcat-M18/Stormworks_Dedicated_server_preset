	...
	evacuate =
	{
		...
		spawn = function(self, mission, location, difficulty_factor, override_transform, min_range, max_range)
			...
			if #safe_zones > 0 and #danger_zones > 0 then
				...
				mission.title = "Evacuate Civilians"
				mission.desc = survivor_count.." civilians need evacuating to a safe zone in anticipation of a natural disaster"
		...
		rebuild_ui = function(self, mission)
			...
			for _, z in pairs(mission.data.danger_zones) do
				if z.survivor_count > 0  then
					...
					addMarker(mission, createMarker(marker_x, marker_z, z.survivor_count.." civilians need evacuating to a safe zone in anticipation of a natural disaster", "", 0, 1), 255, 0, 0, 125)
				end
			end
		...
		terminate = function(self, mission)
			if mission.data.life <= 0 then
				server.notify(-1, "Mission Expired",  "Another rescue service managed to get the civilians to safety.", 2)
			else
				server.notify(-1, "Mission Complete",  mission.desc, 4)
			end
		end,
